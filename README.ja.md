...
?> 元のドキュメントは次のように書かれています[英語](README.md).翻訳は Google 翻訳によって行われます。エラーが見つかった場合は、無視してください。支援したい場合は、プル リクエストを作成できます。
