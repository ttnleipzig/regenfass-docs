...
| ![](_media/hardware/sensor-vl6180x.svg)             | [BL6180X](https://amzn.to/3zVEFPM)飞行时间传感器相对便宜且易于使用。 VL6180X激光测距模块是一款使用激光来测量传感器与物体之间距离的传感器。它是一种飞行时间 (ToF) 传感器，这意味着它测量激光从物体反射并返回传感器所需的时间。该传感器不防水，但精度更高。您必须将其放入防水外壳中。如果您只是想尝试一下，我们推荐该传感器。不建议长期使用。                                                                          |
